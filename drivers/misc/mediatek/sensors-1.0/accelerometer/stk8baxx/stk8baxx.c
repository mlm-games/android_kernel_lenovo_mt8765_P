...
#define STK_ACC_DRIVER_VERSION "404.3.1" /* kernel-version:major-number:minor-number */
...
/* STK8BAXX_REG_BWSEL */
#ifdef STK_INTERRUPT_MODE
#define STK8BAXX_BWSEL_INIT_ODR          0xA    /* ODR = BW x 2 = 62.5Hz */
...
struct stk8baxx_data
{
    ...
    int                         irq1;
    struct workqueue_struct     *alldata_workqueue; /* all data workqueue for int1. */
    struct work_struct          alldata_work;       /* all data work for int1. */
...
static void stk_data_irq_work(struct work_struct *work)
{
    ...
    {
        if (STK8BAXX_INTSTS1_ANY_MOT_STS & data)
        {
...
static int stk_reg_init(struct stk8baxx_data *stk)
{
    ...
    error = stk8baxx_reg_write(stk, STK8BAXX_REG_INTMAP1, STK8BAXX_INTMAP1_ANYMOT2INT1);
    ...
    /*
     * latch int
     * In interrupt mode + any motion mode, both of them share the same INT.
     ...
     * In latch mode, echo interrupt(ANY_MOT_STS) will cause all INT(INT1)
     * rising up.
     */
...
static int stk8baxx_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    hq_regiser_hw_info(HWID_GSENSOR,"stk8ba53");
    STK_ACC_LOG("Success");
    return 0;
