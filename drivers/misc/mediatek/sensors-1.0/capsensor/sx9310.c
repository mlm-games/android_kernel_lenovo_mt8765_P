...
static int sx9310_i2c_suspend(struct device *dev);
static int sx9310_i2c_resume(struct device *dev);
...
static const struct of_device_id sar_of_match[] = {
    ...
    static const struct dev_pm_ops sx9310_i2c_pm_ops = {
        SET_SYSTEM_SLEEP_PM_OPS(sx9310_i2c_suspend, sx9310_i2c_resume)
    };
    #endif
#endif
			   ...
			   struct device_attribute *attr, const char *buf, size_t count)
{
	...
	CAP_LOG("irq gpio val_gpio=%d this->irq =%d\n", val_gpio,this->irq);
								...
								struct device_attribute *attr, char *buf)
{
	...
	dev_info(this->pdev, "sx9311 Reading REG\n");

	for (i=0x10;i<0x2b;i++)
    	{
        	reg_address = i;
       		read_register(this,reg_address,&reg_value);
        	_tLength += snprintf((buf + _tLength), (4096 - _tLength), "Reg[0x%02X]: 0x%02X\n", i, reg_value);
...
static ssize_t sx9310_raw_data_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    for (i = 0; i < 3; i++){
		...
		CAP_LOG("PH[%d]: Useful : %d Average : %d, Offset : %d, DIFF : %d\n",
		 ...
		 _tLength += snprintf((buf + _tLength), (4096 - _tLength), "PH[%d]: Useful : %d Average : %d, Offset : %d, DIFF : %d\n",
				i,useful,average,offset,diff);
...
static ssize_t sx9310_opendev_show(struct device *dev,	struct device_attribute *attr, char *buf)
{
       psx93XX_t this = dev_get_drvdata(dev);
	 int ret = 0;
	if(NULL == this)
	...
	CAP_LOG("sx9310_opendev_show\n");

	ret = write_register(this, sx9310_CPS_CTRL0_REG,0x23);

	return sprintf(buf, "%d\n", ret);
}

static ssize_t sx9310_closedev_show(struct device *dev,	struct device_attribute *attr, char *buf)
{
	psx93XX_t this = dev_get_drvdata(dev);
	int ret = 0;
	if(NULL == this)
...
static DEVICE_ATTR(register_read,0664, sx9310_register_read_show,sx9310_register_read_store);
static DEVICE_ATTR(raw_data,0664,sx9310_raw_data_show,NULL);
static DEVICE_ATTR(chipinfo, 0664, chipinfo_value_show,NULL);
static DEVICE_ATTR(opendev, 0664,sx9310_opendev_show, NULL);
static DEVICE_ATTR(closedev, 0664,sx9310_closedev_show, NULL);
...
static struct attribute *sx9310_attributes[] = {
	...
	&dev_attr_opendev.attr,
	&dev_attr_closedev.attr,
	NULL,
};
...
static int sx9310_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
  ...
  // for dts parase
	...
	if (this){
		...
		* (1->NIRQ=0, 0->NIRQ=1) */
		...
		if (pDevice){
			...
			sx9310_reg_init(this); //modify 20190408
	...
	sx93XX_init(this);
	hq_regiser_hw_info(HWID_CAPSENSOR,"sx9311");
...
void sx93XX_resume(psx93XX_t this)
{
  if (this) {
	...
	//	this->init(this);
	//enable_irq(this->irq);
	write_register(this, sx9310_IRQ_ENABLE_REG,0x60);
  }
}
