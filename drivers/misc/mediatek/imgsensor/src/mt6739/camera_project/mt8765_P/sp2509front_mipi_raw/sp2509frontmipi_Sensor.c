...
#include "sp2509frontmipi_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "SP2509FRONT_camera_sensor"
#define LOG_1 LOG_INF("SP2509FRONT,MIPI 1LANE\n")
...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = SP2509FRONT_SENSOR_ID,

    .checksum_value = 0xafe2bff,        //checksum value for Camera Auto Test

    .pre = {
        .pclk = 36833333,            //record different mode's pclk
...
#define DEBUG_SENSOR
#ifdef DEBUG_SENSOR
	#define sp2509FRONTMIPI_OP_CODE_INI		0x00		/* Initial value. */
	#define sp2509FRONTMIPI_OP_CODE_REG		0x01		/* Register */
	#define sp2509FRONTMIPI_OP_CODE_DLY		0x02		/* Delay */
	#define sp2509FRONTMIPI_OP_CODE_END		0x03		/* End of initial setting. */
	...
	} sp2509FRONTMIPI_initial_set_struct;

	sp2509FRONTMIPI_initial_set_struct sp2509FRONTMIPI_Init_Reg[1000];
 ...
 static u32 strtol(const char *nptr, u8 base)
{
 ...
 u8 sp2509FRONTMIPI_Initialize_from_T_Flash(void)
{
	...
	while (curr_ptr < (data_buff + file_size))
	{
		...
		if (strcmp((const char *)func_ind, "REG") == 0)		/* REG */
		{
			...
			sp2509FRONTMIPI_Init_Reg[i].op_code = sp2509FRONTMIPI_OP_CODE_REG;
			
			sp2509FRONTMIPI_Init_Reg[i].init_reg = strtol((const char *)curr_ptr, 16);
			curr_ptr += 5;	/* Skip "00, 0x" */
		
			sp2509FRONTMIPI_Init_Reg[i].init_val = strtol((const char *)curr_ptr, 16);
			curr_ptr += 4;	/* Skip "00);" */
		...
		else 									/* DLY */
		{
			...
			curr_ptr += 4;	
			sp2509FRONTMIPI_Init_Reg[i].op_code = sp2509FRONTMIPI_OP_CODE_DLY;
			
			sp2509FRONTMIPI_Init_Reg[i].init_reg = 0xFF;
			sp2509FRONTMIPI_Init_Reg[i].init_val = strtol((const char *)curr_ptr,  10);	/* Get the delay ticks, the delay should less then 50 */
		}
		i++;
	...
	sp2509FRONTMIPI_Init_Reg[i].op_code = sp2509FRONTMIPI_OP_CODE_END;
	sp2509FRONTMIPI_Init_Reg[i].init_reg = 0xFF;
	sp2509FRONTMIPI_Init_Reg[i].init_val = 0xFF;
	i++;
	//for (j=0; j<i; j++)
		//printk(" %x  ==  %x\n",sp2509FRONTMIPI_Init_Reg[j].init_reg, sp2509FRONTMIPI_Init_Reg[j].init_val);

	/* Start apply the initial setting to sensor. */
	#if 1
	for (j=0; j<i; j++)
	{
		if (sp2509FRONTMIPI_Init_Reg[j].op_code == sp2509FRONTMIPI_OP_CODE_END)	/* End of the setting. */
		{
			break ;
		}
		else if (sp2509FRONTMIPI_Init_Reg[j].op_code == sp2509FRONTMIPI_OP_CODE_DLY)
		{
			msleep(sp2509FRONTMIPI_Init_Reg[j].init_val);		/* Delay */
		}
		else if (sp2509FRONTMIPI_Init_Reg[j].op_code == sp2509FRONTMIPI_OP_CODE_REG)
		{
		
			write_cmos_sensor(sp2509FRONTMIPI_Init_Reg[j].init_reg, sp2509FRONTMIPI_Init_Reg[j].init_val);
			printk("%x = %x\n",sp2509FRONTMIPI_Init_Reg[j].init_reg,sp2509FRONTMIPI_Init_Reg[j].init_val);
		}
		else
		{
...
static kal_uint16 set_gain(kal_uint16 gain)
{
   	 ...
   	 {   
   	    	 ...
   	    	 if(iReg<=0x10)
   	    	 {
   	    	    	 ...
   	    	    	 write_cmos_sensor(0x01, 0x01);
                      LOG_INF("SP2509FRONTMIPI_SetGain = 16");
   	    	 }
   	    	 else if(iReg>= 0xa0)//gpw
   	    	 {
   	    	    	 ...
   	    	    	 write_cmos_sensor(0x01, 0x01); 
                      LOG_INF("SP2509FRONTMIPI_SetGain = 160"); 
	        }        	
   	    	 else
   	    	 {
   	    	    	write_cmos_sensor(0xfd, 0x01);
   	    	    	...
   	    	    	write_cmos_sensor(0x01, 0x01);
			LOG_INF("SP2509FRONTMIPI_SetGain = %d",iReg);		 
	       }	
   	 }	
   	 else
...
static void sensor_init(void)
{
	...
	if(fromsd == 1)//是否从SD读取//gepeiwei   120903
		...
		sp2509FRONTMIPI_Initialize_from_T_Flash();//从SD卡读取的主要函数
...
static kal_uint32 open(void)
{
    ...
    if (imgsensor_info.sensor_id != sensor_id)
	    ...
	    {
		    fromsd = 1;
	          //sp2509FRONTMIPI_Initialize_from_T_Flash();
	          printk("gpww read ok!\n");
...
UINT32 SP2509FRONT_MIPI_RAW_SensorInit(SENSOR_FUNCTION_STRUCT **pfFunc)
{
    /* To Do : Check Sensor status here */
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
}    /*    SP2509FRONTMIPISensorInit    */
