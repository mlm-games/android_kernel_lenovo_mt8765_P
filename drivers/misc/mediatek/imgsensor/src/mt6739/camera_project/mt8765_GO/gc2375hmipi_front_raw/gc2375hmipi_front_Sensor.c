...
#include "gc2375hmipi_front_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "gc2375h_front_camera_sensor"
#define LOG_1 LOG_INF("GC2375H_FRONT,MIPI 1LANE\n")
/****************************   Modify end    *******************************************/

#define LOG_INF(format, args...)    pr_debug(PFX "[%s] " format, __FUNCTION__, ##args)

#define GC2375H_FRONT_MIPI_USE_OTP

static DEFINE_SPINLOCK(imgsensor_drv_lock);
kal_bool GC2375HFRONTDuringTestPattern = KAL_FALSE;

static imgsensor_info_struct imgsensor_info = {
    .sensor_id = GC2375HMIPI_FRONT_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h

    .checksum_value = 0x44176ee9,        //checksum value for Camera Auto Test
...
#ifdef GC2375H_FRONT_MIPI_USE_OTP
...
static void gc2375h_gcore_read_otp_info(void)
{
	...
	LOG_INF("GC2375H_FRONT_OTP : flag = 0x%x\n",flag);
	...
	{
		LOG_INF("GC2375H_FRONT_OTP_WB:Valid");
		/*	
		...
		*/
		...
		LOG_INF("GC2375H_FRONT_OTP_WB:r_gain=0x%x\n",gc2375h_otp_info.r_gain);
		LOG_INF("GC2375H_FRONT_OTP_WB:g_gain=0x%x\n",gc2375h_otp_info.g_gain);
		LOG_INF("GC2375H_FRONT_OTP_WB:b_gain=0x%x\n",gc2375h_otp_info.b_gain);		
		LOG_INF("GC2375H_FRONT_OTP_WB:g_r_gain_l=0x%x\n",gc2375h_otp_info.g_r_gain_l);
		LOG_INF("GC2375H_FRONT_OTP_WB:b_g_gain_l=0x%x\n",gc2375h_otp_info.b_g_gain_l);
	}
	else
	{
		LOG_INF("GC2375H_FRONT_OTP_WB:Invalid!");
	}

	/*OB*/
	if(0x01==gc2375h_otp_info.ob_flag)
	{
		LOG_INF("GC2375H_FRONT_OTP_OB:Valid");
		/*
		...
		*/
		...
		LOG_INF("GC2375H_FRONT_OTP_OB:g1_slope=0x%x\n",gc2375h_otp_info.g1_slope);
		LOG_INF("GC2375H_FRONT_OTP_OB:r1_slope=0x%x\n",gc2375h_otp_info.r1_slope);
		LOG_INF("GC2375H_FRONT_OTP_OB:b2_slope=0x%x\n",gc2375h_otp_info.b2_slope);	
		LOG_INF("GC2375H_FRONT_OTP_OB:g2_slope=0x%x\n",gc2375h_otp_info.g2_slope);		
		LOG_INF("GC2375H_FRONT_OTP_OB:ob_slope=0x%x\n",gc2375h_otp_info.ob_slope);			
	}
	else
	{
		LOG_INF("GC2375H_FRONT_OTP_OB:Invalid!");
	}	
...
static void gc2375h_gcore_enable_otp(otp_state state)
{
	...
	{ 
		...
		LOG_INF("GC2375H_FRONT_OTP: Enable OTP!\n");		
	}
	else			
	{
		otp_en = otp_en & 0x7f;
		...
		LOG_INF("GC2375H_FRONT_OTP: Disable OTP!\n");
	}

}

void gc2375h_front_gcore_identify_otp(void)
{	
	write_cmos_sensor(0xfe, 0x00);				
	write_cmos_sensor(0xfe, 0x00);						  
	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0xf7, 0x03); //01 -> 03
	write_cmos_sensor(0xf8, 0x1b); // 0c -> 1b
...
static kal_uint32 open(void)
{
...
#ifdef GC2375H_FRONT_MIPI_USE_OTP
    gc2375h_front_gcore_identify_otp();	
#endif
    ...
    spin_unlock(&imgsensor_drv_lock);
    GC2375HFRONTDuringTestPattern = KAL_FALSE; 
...
UINT32 GC2375HMIPI_FRONT_RAW_SensorInit(SENSOR_FUNCTION_STRUCT **pfFunc) 
{
