...
static int thub_polling_thread(void *arg)
{
	...
	while (1) {
		...
		if (i_ACA_type == 4 && i_ACA_type_last == 0) {
			pr_err("Adaptor plug in\n");
			pr_err("disable charger otg\n");
			charger_dev_enable_otg(primary_charger, false);
			pr_err("call charger detect\n");
			do_charger_detect();
		} else if ((i_ACA_type == 0 && i_ACA_type_last == 4) || i_ACA_type == 8) {
			pr_err("Adaptor plug out\n");
			pr_err("call charger detect\n");
...
static void musb_host_work(struct work_struct *data)
{
	...
	DBG(0, "csdk otg_enable = %d\n", mtk_musb->otg_enable);

	if (mtk_musb->otg_enable && iddig_req_host) {
		host_mode = 1;
		mtk_musb->is_host = 0;
	} else {
		host_mode = 0;
