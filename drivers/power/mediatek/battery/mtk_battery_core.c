...
static enum hq_batteryID hq_get_battery_id(void)
{
	...
	ret = gpiod_get_raw_value(gpio_to_desc(BAT_ID_PIN));

	pr_err("[%s] ret = %d, BAT_ID_PIN = %d\n", __func__, ret, BAT_ID_PIN);
	if (ret){
		gm.battery_id = HQ_BATTERY_ATL;
		g_battery_manufacturer = "XWD-ATL";
	} else {
		gm.battery_id = HQ_BATTERY_COS;
...
void fg_custom_init_from_dts(struct platform_device *dev)
{
	...
	BAT_ID_PIN = of_get_named_gpio(np, "bat_id_pin", 0);
	gpio_request(BAT_ID_PIN, "BAT_ID_PIN");

	/* fgauge_get_profile_id(); */
	/* bat_id = gm.battery_id; */
	bat_id = hq_get_battery_id();

	if (bat_id == HQ_BATTERY_ATL)
		hq_regiser_hw_info(HWID_BATTERY_VENDOR,"Scud");
	else if (bat_id == HQ_BATTERY_COS)
		hq_regiser_hw_info(HWID_BATTERY_VENDOR,"Sunwoda");
	else
