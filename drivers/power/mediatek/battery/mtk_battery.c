...
#define MTK_OVERHEAT_TEMP 50
#define MTK_COLD_TEMP     0
...
static ssize_t show_Charger_enable(struct device *dev, struct device_attribute *attr, char *buf)
{
	unsigned int chr_enable = 0;
	/* chr_enable = (battery_main.BAT_STATUS == POWER_SUPPLY_STATUS_CHARGING)?1:0; */
	chr_enable = g_chg_en_flag;
	bm_err("CHARGER_ENABLE = %d\n", chr_enable);
	return sprintf(buf, "%u\n", chr_enable);
}

static ssize_t store_Charger_enable(struct device *dev, struct device_attribute *attr,
									const char *buf, size_t size)
{
	int rv;
	...
	rv = kstrtouint(buf, 0, &g_chg_en_flag);
	if (rv != 0)
		return -EINVAL;
	...
	else if (g_chg_en_flag == 1 && chr_type != CHARGER_UNKNOWN)
...
static DEVICE_ATTR(Charger_enable, 0664, show_Charger_enable, store_Charger_enable);
...
/*****************************/
	...
	ret = device_create_file(&(dev->dev), &dev_attr_Charger_enable);
