...
static int bq2560x_set_led_stat(struct charger_device *chg_dev, int ctrl)
{
	struct bq2560x *bq = dev_get_drvdata(&chg_dev->dev);

	return bq2560x_set_stat_ctrl(bq, ctrl);
}
...
static int bq2560x_init_device(struct bq2560x *bq)
{
	...
	pr_err("[Lingfei][%s][%d] bq->platform_data->statctrl = %d\n", __func__,
			__LINE__, bq->platform_data->statctrl);
...
static int bq2560x_charging(struct charger_device *chg_dev, bool enable)
{
	...
	if (enable) {
		...
		if (g_hiz_mode) {
			...
			pr_err("[%s] bq2560x enter hiz mode\n", __func__);
		} else {
			bq2560x_exit_hiz_mode(bq);
			pr_err("[%s] bq2560x exit hiz mode\n", __func__);
		}
...
//	.set_ta20_reset = NULL,
	...
	/* set red led */
	.set_led_stat = bq2560x_set_led_stat,
